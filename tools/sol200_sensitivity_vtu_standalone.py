...
RUN_CONFIG = {
    ...
    "output_bdf": r"D:/WorkSpace/OtherProjects/VirtualReal702/run/fem15_all_elem_e_sol200000000.bdf",
    "parameter_preset": {
...
def _build_sol200_control_lines(settings: Dict[str, Any], sensitivity_csv_internal: Optional[str]) -> List[str]:
    ...
    if sensitivity_csv_internal:
        lines.append(f"ASSIGN USERFILE='{Path(sensitivity_csv_internal).name}' FORM=FORMATTED STATUS=NEW UNIT=52")
    lines.extend(
        [
            "SOL 200",
            "CEND",
            "METHOD = 1",
            "DISPLACEMENT(OP2)=ALL",
            "DESSUB = 1",
            ...
            f"EIGRL   {1:>8}{fmin:>8}{fmax:>8}{vectors:>8}{'':>8}{'':>8}{'':>8}{dynamic_norm:>8}",
        ]
    )
