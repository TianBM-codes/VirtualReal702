...
@router.post("/fem/modal/frequencies")
async def modal_frequencies_api(request: Request, body: ModalFrequenciesRequest):
    await log_request(request, model_to_dict(body))
    try:
        result = get_project_modal_frequencies_payload(project_id=body.project_id)
        return success_response(result, "获取所有计算频率成功")
    except AppError as exc:
        return error_response(exc.status_code, exc.message, error_code=exc.code, details=exc.details)
    except Exception as exc:
        app_exc = server_error(exc)
