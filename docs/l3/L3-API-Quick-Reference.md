...
| results | POST | `/api/odb/{odb_id}/results/warmup` |
...
- **"找不到"不再返回 404，而是静默返回 200 + 空 payload**：当 ODB/instance/step/field/result_group 任一不存在（原本会抛 `NotFoundError`→404）时，改为返回结构完全一致的空响应——`u_per_vertex` 长度为 0、`legend_range=[0,0]`，响应头字段与正常响应一致（不加任何特殊标记头）。这样前端把同一查询广播给所有 instance 时，缺该结果的 instance 解析出空数组、按无数据渲成灰色，而不会因为 404 让整批 `Promise.all` 失败。注意：仍在处理中（`NotReadyError`→202）和参数非法（`ValidationError`→400）**不受影响**，照常返回原状态码。
...
### `POST /api/odb/{odb_id}/results/warmup`

手动触发该模型的**缓存预热**：后台线程预算每个 result_group × step × field × instance 的第 0 帧默认视图（模长分量）及其归一化范围，写入内存 + 磁盘缓存。接口立即返回，不等预热完成。幂等——已缓存的组合直接跳过，重复调用只补算新增字段。

无请求体。响应：

```json
{ "code": 200, "data": { "odb_id": "...", "warmup": "started" }, "message": "" }
```

- `warmup`：`started`（已启动后台预热）或 `already_running`（该模型已有预热在跑）。
- odb_id/project_id 未注册时返回 404。
- **正常情况下不需要调用**：模型加载完成、poll 线程发现新 result_group 就绪时会自动预热（`APP_WARMUP=0` 可关闭自动预热，本接口不受影响）。预热进度看服务日志的 `Warmup[...]` 行。
