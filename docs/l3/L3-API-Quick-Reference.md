...
FREQUENCY/BUCKLE 步：显式传这类 step 时，"time" 轴实际是 `frame_value`（模态阶次/频率/特征值，不是时间）。`exact`/`prev`/`next` 按该轴取最近一帧有意义、照常放行；`interp` 落在两帧之间时返回 400（两个模态振型线性插值无物理意义），提示改用 `prev`/`next` 或直接传某帧的 `frame_value`。
