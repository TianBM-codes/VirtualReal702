...
- **Result caching in L3（两级缓存 + 预热）:** frame-scalars 归一化前的 scalar_vertex 与图例范围按 (结果文件 mtime+size, instance, 帧, 分量, 渲染参数) 缓存：进程内 LRU（`APP_SCALAR_CACHE_MB`，默认 512MB/worker，0=关）+ 工作区磁盘 `<workspace>/l3_cache/`（`APP_SCALAR_DISK_CACHE_MB`，默认 2048MB，0=关；重启不丢、多 worker 共享，是"L3 只写 manifest.db"约定的唯一例外，随工作区删除）。结果文件被重写即自动失效；set 过滤 / override 归一化不缓存，仍按请求执行。模型加载完成 / 新 result_group 就绪时后台预热第 0 帧默认视图（`APP_WARMUP`，`services/warmup_service.py`，手动触发用 `POST /results/warmup`）。
